...
# Old and new configuration paths
...
CONFIG_FILE_JSON = os.path.join(NEW_CONFIG_DIR, "xPand.json")
OLD_CONFIG_FILE_JSON = os.path.join(OLD_CONFIG_DIR, "xPand.json")
OLD_CONFIG_FILE_JSONL = os.path.join(OLD_CONFIG_DIR, "xPand.jsonl")
...
class XPandData:
	...
	def load(self):
		with self._lock:
			...
			if os.path.exists(CONFIG_FILE_JSON):
				try:
					with codecs.open(CONFIG_FILE_JSON, 'r', encoding='utf-8') as f:
						data = json.load(f)
						if isinstance(data, dict):
							self.shortcuts = data
							log.info("Loaded shortcuts from new JSON format")
							return
				except Exception as e:
					log.error(f"Error loading new JSON file: {e}")
			
			# Check for old files and migrate them
			migrated = False
			if os.path.exists(OLD_CONFIG_FILE_JSON):
				try:
					# Move the old JSON file to the new location
					shutil.move(OLD_CONFIG_FILE_JSON, CONFIG_FILE_JSON)
					log.info("Migrated old JSON file to new location")
					migrated = True
				except Exception as e:
					log.error(f"Failed to migrate old JSON file: {e}")
			
			if not migrated and os.path.exists(OLD_CONFIG_FILE_JSONL):
				try:
					# Convert old JSONL format and save to new location
					log.info("Found old JSONL format, converting and migrating")
					self.convert_old_format()
					migrated = True
				except Exception as e:
					log.error(f"Failed to convert old JSONL format: {e}")
			
			if migrated:
				# Try loading again from new location
				try:
					with codecs.open(CONFIG_FILE_JSON, 'r', encoding='utf-8') as f:
						data = json.load(f)
						if isinstance(data, dict):
							self.shortcuts = data
							log.info("Loaded shortcuts after migration")
							return
				except Exception as e:
					log.error(f"Error loading migrated JSON: {e}")
	...
	def convert_old_format(self):
		"""Convert from old JSONL format to new simple JSON format"""
		try:
			old_shortcuts = {}
			with codecs.open(OLD_CONFIG_FILE_JSONL, 'r', encoding='utf-8') as f:
				for line in f:
					line = line.strip()
					if line:
						try:
							...
							for code, expansions in entry.items():
								# Extract text from old nested format [[text]]
								if isinstance(expansions, list) and expansions and isinstance(expansions[0], list):
									old_shortcuts[code] = expansions[0][0] if expansions[0] else ""
								else:
									old_shortcuts[code] = str(expansions)
						except json.JSONDecodeError:
							continue
			
			# Save in new format to new location
			self.shortcuts = old_shortcuts
			self.save()
			
			# Backup old file
			try:
				os.rename(OLD_CONFIG_FILE_JSONL, OLD_CONFIG_FILE_JSONL + ".backup")
				log.info("Converted old JSONL format to new JSON format")
			except Exception as e:
				log.error(f"Error backing up old file: {e}")
				
		except Exception as e:
			log.error(f"Error converting old format: {e}")
			self.shortcuts = self.get_defaults()
			self.save()
	...
	def save(self):
		with self._lock:
			try:
				# Ensure directory exists (should already, but just in case)
				os.makedirs(NEW_CONFIG_DIR, exist_ok=True)
				with codecs.open(CONFIG_FILE_JSON, 'w', encoding='utf-8') as f:
...
class XPandSettingsDialog(wx.Dialog):
	...
	def init_ui(self):
		...
		# Enable paste for code input
		self.code_input.Bind(wx.EVT_CONTEXT_MENU, self.on_context_menu_code)
		...
		# Enable paste for word input
		self.word_input.Bind(wx.EVT_CONTEXT_MENU, self.on_context_menu_word)
		...
		self.context_menu_add = self.context_menu.Append(-1, _("&Add"))
		self.context_menu_edit = self.context_menu.Append(-1, _("&Edit"))
		self.context_menu_remove = self.context_menu.Append(-1, _("&Remove"))
		...
		# Create context menus for text controls
		self.context_menu_code = wx.Menu()
		self.context_menu_code_cut = self.context_menu_code.Append(-1, _("Cu&t"))
		self.context_menu_code_copy = self.context_menu_code.Append(-1, _("&Copy"))
		self.context_menu_code_paste = self.context_menu_code.Append(-1, _("&Paste"))
		self.context_menu_code.AppendSeparator()
		self.context_menu_code_select_all = self.context_menu_code.Append(-1, _("Select &All"))
		
		self.context_menu_word = wx.Menu()
		self.context_menu_word_cut = self.context_menu_word.Append(-1, _("Cu&t"))
		self.context_menu_word_copy = self.context_menu_word.Append(-1, _("&Copy"))
		self.context_menu_word_paste = self.context_menu_word.Append(-1, _("&Paste"))
		self.context_menu_word.AppendSeparator()
		self.context_menu_word_select_all = self.context_menu_word.Append(-1, _("Select &All"))
		
		self.Bind(wx.EVT_MENU, self.on_cut_code, self.context_menu_code_cut)
		self.Bind(wx.EVT_MENU, self.on_copy_code, self.context_menu_code_copy)
		self.Bind(wx.EVT_MENU, self.on_paste_code, self.context_menu_code_paste)
		self.Bind(wx.EVT_MENU, self.on_select_all_code, self.context_menu_code_select_all)
		
		self.Bind(wx.EVT_MENU, self.on_cut_word, self.context_menu_word_cut)
		self.Bind(wx.EVT_MENU, self.on_copy_word, self.context_menu_word_copy)
		self.Bind(wx.EVT_MENU, self.on_paste_word, self.context_menu_word_paste)
		self.Bind(wx.EVT_MENU, self.on_select_all_word, self.context_menu_word_select_all)
		
		self.update_button_states()
		self.code_list.SetFocus()
	
	def on_context_menu_code(self, event):
		self.code_input.PopupMenu(self.context_menu_code)
	
	def on_context_menu_word(self, event):
		self.word_input.PopupMenu(self.context_menu_word)
	
	def on_cut_code(self, event):
		self.code_input.Cut()
	
	def on_copy_code(self, event):
		self.code_input.Copy()
	
	def on_paste_code(self, event):
		self.code_input.Paste()
	
	def on_select_all_code(self, event):
		self.code_input.SelectAll()
	
	def on_cut_word(self, event):
		self.word_input.Cut()
	
	def on_copy_word(self, event):
		self.word_input.Copy()
	
	def on_paste_word(self, event):
		self.word_input.Paste()
	
	def on_select_all_word(self, event):
	...
	def refresh_list(self, filter_text=""):
		...
		self.all_codes = [(k, str(v)) for k, v in codes.items()]
	...
	def on_char(self, event):
		...
		# Check if it's a printable character (not control key)
		if unicode_key != wx.WXK_NONE and unicode_key >= 32 and unicode_key != 127:
			current_time = time.time()
			char = chr(unicode_key)
			
			# Reset prefix if too much time has passed (1 second)
			...
			# Find the first code starting with the prefix (case insensitive)
			found_index = -1
			for i in range(self.code_list.GetItemCount()):
				code = self.code_list.GetItemText(i, 0)
				if code.lower().startswith(self.jump_word_prefix.lower()):
					found_index = i
					break
			
			if found_index >= 0:
				self.code_list.Select(found_index)
				self.code_list.Focus(found_index)
				self.code_list.EnsureVisible(found_index)
				code_text = self.code_list.GetItemText(found_index, 0)
				expansion_text = self.code_list.GetItemText(found_index, 1)
				ui.message(_("{code}: {expansion}").format(code=code_text, expansion=expansion_text))
			else:
				# If no exact match, try partial match (contains)
				found_index = -1
				for i in range(self.code_list.GetItemCount()):
					code = self.code_list.GetItemText(i, 0)
					if self.jump_word_prefix.lower() in code.lower():
						found_index = i
						break
				...
				if found_index >= 0:
					...
					# Remove last character if no match found
					self.jump_word_prefix = self.jump_word_prefix[:-1]
					if self.jump_word_prefix:
						ui.message(_("No code starting with: {prefix}").format(prefix=self.jump_word_prefix))
					else:
						ui.message(_("No matching code"))
		else:
			# For non-printable keys (arrows, etc.), let the event propagate
			event.Skip()
	...
	def on_add(self, event):
		...
		if self.xpand_data.get(code) is not None:
...
class GlobalPlugin(globalPluginHandler.GlobalPlugin):
	...
	def process_gesture(self, gesture_time):
		if self.gesture_count == 1:
			# Single tap - open main dialog if in text editor or web browser
			if self.is_text_editor() or self.is_web_browser_field():
				self.open_xpand_dialog()
			else:
				ui.message(_("Not in an editable field"))
	...
	def is_text_editor(self):
		...
		try:
			...
			# Check if we can get text info (indicates editable field)
			try:
				text_info = focus.makeTextInfo(textInfos.POSITION_CARET)
				if text_info:
					return True
			except:
				pass
	...
	def is_web_browser_field(self):
		"""Check if focused field is in a web browser and is editable"""
		try:
			focus = api.getFocusObject()
			if not focus:
				return False
			...
			# Common web browser process names and window classes
			web_browsers = [
				'chrome.exe', 'firefox.exe', 'msedge.exe', 'brave.exe',
				'opera.exe', 'safari.exe', 'iexplore.exe', 'applicationframehost.exe'
			]
			
			is_browser = any(browser in process_name for browser in web_browsers)
			
			# Also check window class for browser identifiers
			browser_window_classes = [
				'chrome_widgetwin', 'mozillawindowclass', 'ieframe',
				'applicationframewindow', 'operawindowclass'
			]
			
			is_browser = is_browser or any(cls in window_class for cls in browser_window_classes)
			
			if not is_browser:
				return False
			
			# Check if the focused element is editable
			# First try to get text info - this is the most reliable method
			try:
				text_info = focus.makeTextInfo(textInfos.POSITION_CARET)
				if text_info:
					return True
			...
			# Look for specific roles that indicate editable fields
			editable_roles = [
				controlTypes.Role.EDITABLETEXT,
				controlTypes.Role.TEXTFRAME,
			]
			
			# Check if focus has editable role
			if hasattr(focus, 'role') and focus.role in editable_roles:
				return True
			
			# Check for specific states that indicate editable fields
			if hasattr(focus, 'states'):
				# EDITABLE state indicates the field can be edited
				if controlTypes.State.EDITABLE in focus.states:
					return True
				
				# FOCUSED and FOCUSABLE states often indicate editable fields in browsers
				if (controlTypes.State.FOCUSED in focus.states and 
					controlTypes.State.FOCUSABLE in focus.states):
					return True
			
			# Additional check for common web element roles and attributes
			# Many web form fields will have these characteristics
			try:
				# Check for specific IAccessible roles that indicate editable fields
				if hasattr(focus, 'IAccessibleObject'):
					acc_role = focus.IAccessibleObject.accRole(0)
			...
			# Check for specific ARIA roles or attributes that indicate editable fields
			if hasattr(focus, 'IA2Attributes'):
				attributes = focus.IA2Attributes
				if attributes:
					# Look for contenteditable="true" or roles that indicate editable text
					for attr in attributes:
						if isinstance(attr, dict):
							for key, value in attr.items():
								if ('contenteditable' in key.lower() and 'true' in value.lower()) or \
								   ('role' in key.lower() and any(editable_role in value.lower() for editable_role in ['textbox', 'searchbox', 'combobox', 'textarea'])):
									return True
						elif isinstance(attr, str):
							if ('contenteditable' in attr and 'true' in attr) or \
							   ('role' in attr and any(editable_role in attr for editable_role in ['textbox', 'searchbox', 'combobox', 'textarea'])):
								return True
			
			# Check if the control is a password field (common in login forms)
			if hasattr(focus, 'IA2Attributes'):
				attributes = focus.IA2Attributes
				if attributes:
					for attr in attributes:
						if isinstance(attr, dict):
							for key, value in attr.items():
								if 'type' in key.lower() and 'password' in value.lower():
									return True
						elif isinstance(attr, str):
							if 'type' in attr and 'password' in attr:
								return True
			
			# Final check - if it's a browser and we can type, allow it
			# This is a more permissive check for browser input fields
			try:
				# Check if the object has a value attribute (common for input fields)
				if hasattr(focus, 'value'):
					return True
				
				# Check if it's a search field or text area
				if hasattr(focus, 'role'):
					role_name = controlTypes.role._roleLabels.get(focus.role, "")
					if any(name in role_name.lower() for name in ['edit', 'text', 'search', 'input']):
						return True
			except:
				pass
			...
			log.error(f"Error checking web browser field: {e}")
			return False
